...
import { Folder, Trash2 as TrashIcon, Pencil as PencilIcon } from "lucide-react";
  ...
  return (
    <ContextMenu>
        ...
        >
          ...
          <div className="flex items-center justify-between text-[11px] text-muted-foreground/80 tabular-nums">
            <div className="flex items-center gap-1.5">
               ...
               {noteCount > 0 && folderCount > 0 && <span className="opacity-40">•</span>}
               {folderCount > 0 && (
      ...
      <ContextMenuContent className="w-48">
        <ContextMenuItem onClick={() => openRename(folder.path, folder.name, true)}>
          <PencilIcon className="mr-2 h-4 w-4" />
          Rename
        </ContextMenuItem>
        <ContextMenuSeparator />
        <ContextMenuItem variant="destructive" onClick={() => openDelete(folder.path, folder.name, true)}>
