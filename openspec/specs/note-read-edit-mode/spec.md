# note-read-edit-mode Specification

## Purpose
TBD - created by archiving change add-note-read-edit-mode. Update Purpose after archive.
## Requirements
...
#### Scenario: Double-click on whitespace (no content) enters edit mode at last known position
- **WHEN** user double-clicks on whitespace area (no text under cursor) while in read mode
- **THEN** the editor switches to edit mode
- **AND** cursor position is set to the last known edit position (if available)
- **OR** cursor position is set to the start of the document (if no prior edit position exists)
